...
For a set of $N$ areal units, the relationship between areal units is described by an $n \times n$ adjacency matrix $W$. The entries indicate whether two regions $n_i$ and $n_j$ are neighbors, with a value of 1 signifying adjacency and 0 indicating non-adjacency. It's worth noting that in models like Conditional Autoregressive (CAR) models, the neighbor relationship is symmetric, but a region is not considered its own neighbor ($W_{ii} = 0$).
  ...
  * we can add the pic to demo the structure for neighborhood
...
us_data %>% filter(POPGPW_2005_40 !=0) %>% 
...
ggplot() +
  ...
  geom_sf(data = joined_data,
          aes(fill = log(MER2005_40)), color = NA) +
  theme_classic()+
  theme(legend.title = element_text(size = 10),
               legend.text = element_text(size = 10))+
  labs(fill = "log(Grid Gross Cell \n Product in 2005)")
...
joined_us_data <- joined_us_data %>% na.omit()
form <- `log(MER2005_40)`  ~ dis_to_water + log(POPGPW_2005_40)+  TEMPAV_8008+ PRECAVNEW80_08+D3
...
```{r}
# simply model
simple_fom <-  `log(MER2005_40)`  ~  `log(POPGPW_2005_40)`+ TEMPAV_8008 
simple_data_A
...
simple_data_A$PM <- chain_simple_2$summary.results[ ,"Mean"]
simple_data_A$LL <-  chain_simple_2$summary.results[, "2.5%"]
simple_data_A$UL <- chain_simple_2$summary.results[, "97.5%"]

  
at <- seq(min(c(simple_data_A$PM, simple_data_A$LL, simple_data_A$UL)),
          max(c(simple_data_A$PM, simple_data_A$LL, simple_data_A$UL)),
          length.out = 8)

popuptable <- leafpop::popupTable(dplyr::mutate_if(simple_data_A,
                                  is.numeric, round, digits = 2),
zcol = c("`log(MER2005_40)`", "TEMPAV_8008", "PM", "LL", "UL"),
